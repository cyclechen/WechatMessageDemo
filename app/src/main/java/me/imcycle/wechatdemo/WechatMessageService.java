...
public class WechatMessageService extends AccessibilityService {
    ...
    public void onAccessibilityEvent(final AccessibilityEvent event) {
        ...
        android.util.Log.e("maptrix", "get event = " + eventType);
        switch (eventType) {
            case AccessibilityEvent.TYPE_NOTIFICATION_STATE_CHANGED:// 通知栏事件
                android.util.Log.e("maptrix", "get notification event");
                    ...
                    for (CharSequence text : texts) {
                        ...
                        if (!TextUtils.isEmpty(content)) {
                            if (isScreenLocked()) {
                                locked = true;
                                wakeAndUnlock();
                                android.util.Log.e("maptrix", "the screen is locked");
                                if (isAppForeground(MM_PNAME)) {
                                    background = false;
                                    android.util.Log.e("maptrix", "is mm in foreground");
...
//                                            }
                                    ...
                                    background = true;
                                    android.util.Log.e("maptrix", "is mm in background");
                                    sendNotifacationReply(event);
                                }
                            } else {
                                locked = false;
                                android.util.Log.e("maptrix", "the screen is unlocked");
                                if (isAppForeground(MM_PNAME)) {
                                    background = false;
                                    android.util.Log.e("maptrix", "is mm in foreground");
                                    sendNotifacationReply(event);
...
//                                    }, 1000);
            ...
            case AccessibilityEvent.TYPE_WINDOW_STATE_CHANGED:
                android.util.Log.e("maptrix", "get type window down event");
                if (!hasAction) break;
                itemNodeinfo = null;
...
//                release();
    ...
    private void send() {
        ...
        if (nodeInfo != null) {
            ...
            list = ListUtils.isEmpty(list) ? nodeInfo.findAccessibilityNodeInfosByText("Send") : list;
...
//        }
    ...
    private boolean findEditText(AccessibilityNodeInfo rootNode, String content) {
        ...
        android.util.Log.e("maptrix", "root class=" + rootNode.getClassName() + "," + rootNode.getText() + "," + count);
        for (int i = 0; i < count; i++) {
            AccessibilityNodeInfo nodeInfo = rootNode.getChild(i);
            if (nodeInfo == null) {
                android.util.Log.e("maptrix", "nodeinfo = null");
                continue;
            }

            android.util.Log.e("maptrix", "class=" + nodeInfo.getClassName());
...
//        startActivity(home);
    ...
    private void release() {
        ...
        if (locked && kl != null) {
            android.util.Log.e("maptrix", "release the lock");
            //得到键盘锁管理器对象
            kl.reenableKeyguard();
            locked = false;
        }
    ...
    public void startWechatChating() {
    ...
    public void gotoContact() {
        ...
        if (nodeInfo != null) {
            ...
            list = (list != null && list.size() > 0) ? list : nodeInfo.findAccessibilityNodeInfosByText("Contact");
            if (!ListUtils.isEmpty(list)) {
                for (AccessibilityNodeInfo n : list) {
                    if (n.getClassName().equals("android.widget.TextView") && n.isEnabled()) {
                        n.performAction(AccessibilityNodeInfo.ACTION_CLICK);
                        AccessibilityNodeInfo nodeInfo1 = n.getParent();
        ...
        List<AccessibilityNodeInfo> user = nodeInfo.findAccessibilityNodeInfosByText("Cycle");
        if (!ListUtils.isEmpty(user)) {
            for (AccessibilityNodeInfo n : user) {
                if (n.getClassName().equals("android.view.View") && n.isEnabled()&& n.getText().equals("Cycle")) {
                    n.performAction(AccessibilityNodeInfo.ACTION_CLICK);
                    AccessibilityNodeInfo nodeInfo1 = n.getParent();
                    nodeInfo1.performAction(AccessibilityNodeInfo.ACTION_CLICK);
                }
